...
func (a *ProviderAPI) Create(p domain.Provider) (*domain.Provider, error) {
    ...
    if p.Type == "" || p.Name == "" { return nil, errors.New("type and name are required") }
...
func (a *ProviderAPI) Update(p domain.Provider) (*domain.Provider, error) {
    ctx := context.Background()
    if p.ID == 0 { return nil, errors.New("id is required") }
...
func (a *ProviderAPI) List() ([]*domain.Provider, error) {
    ...
    if err != nil { return nil, err }
    for _, p := range list { p.APIKey = mask(p.APIKey) }
    return list, nil
}

type ModelInfo struct { Name, Description string; ContextTokens int }
...
func (a *ProviderAPI) ListModels(id int64) ([]ModelInfo, error) {
    ...
    if !ok { return nil, errors.New("unsupported provider type") }
    models, err := prov.ListModels(ctx)
    if err != nil { return nil, err }
    out := make([]ModelInfo, 0, len(models))
    for _, m := range models { out = append(out, ModelInfo{Name: m.Name, Description: m.Description, ContextTokens: m.ContextTokens}) }
    return out, nil
}
...
func (a *ProviderAPI) Test(id int64) (ProviderTestResult, error) {
    ctx := context.Background()
    p, err := a.repo.Get(ctx, id)
    if err != nil { return ProviderTestResult{}, err }
    prov, ok := factory.FromProvider(p)
    if !ok { return ProviderTestResult{}, errors.New("unsupported provider type") }
    ...
    user := "Text: hello"
    seg := ports.Segment{Key: "test", Text: "hello"}
    res, trErr := prov.Translate(ctx, seg, ports.TranslateParams{SourceLang: "en", TargetLang: "ru", Model: p.Model, Temperature: 0.0, SystemPrompt: system, UserPrompt: user})
    if trErr != nil {
        return ProviderTestResult{Ok: false, Error: trErr.Error()}, nil
    }
    return ProviderTestResult{Ok: true, Translation: res.Translation, Raw: res.Raw}, nil
}
...
func (a *ProviderAPI) normalizeModel(ctx context.Context, p *domain.Provider) error {
    if p == nil { return nil }
    if strings.ToLower(p.Type) != "openrouter" { return nil }
...
func (a *ProviderAPI) Delete(id int64) (bool, error) {
    ctx := context.Background()
    if err := a.repo.Delete(ctx, id); err != nil { return false, err }
    return true, nil
}
