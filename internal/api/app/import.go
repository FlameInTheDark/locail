...
type UnitKV struct { Key string `json:"key"`; Source string `json:"source"`; Context string `json:"context,omitempty"` }
type ParseResponse struct { Locale string `json:"locale"`; Items []UnitKV `json:"items"` }

func (a *ImportAPI) ParseBase64(req ImportRequest) (ParseResponse, error) {
    b, err := base64.StdEncoding.DecodeString(req.ContentB64)
    if err != nil { return ParseResponse{}, err }
    parser, ok := a.svc.ParserRegistry.Get(req.Format)
    if !ok { return ParseResponse{}, errors.New("unsupported format: "+req.Format) }
