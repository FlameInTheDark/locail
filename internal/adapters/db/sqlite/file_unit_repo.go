...
func (r *FileRepo) Get(ctx context.Context, id int64) (*domain.File, error) {
	...
	var f domain.File
	var created string
	if err := row.Scan(&f.ID, &f.ProjectID, &f.Path, &f.Format, &f.Locale, &f.Hash, &created); err != nil {
		return nil, err
	}
...
func (r *FileRepo) ListByProject(ctx context.Context, projectID int64) ([]*domain.File, error) {
	...
	for rows.Next() {
		var f domain.File
		var created string
		if err := rows.Scan(&f.ID, &f.ProjectID, &f.Path, &f.Format, &f.Locale, &f.Hash, &created); err != nil {
			return nil, err
		}
		f.CreatedAt, _ = time.Parse(time.RFC3339, created)
		out = append(out, &f)
...
func (r *UnitRepo) ListByFile(ctx context.Context, fileID int64) ([]*domain.Unit, error) {
	...
	for rows.Next() {
		var u domain.Unit
		var created string
		if err := rows.Scan(&u.ID, &u.FileID, &u.Key, &u.SourceText, &u.Context, &u.MetadataRaw, &created); err != nil {
			return nil, err
		}
...
func (r *UnitRepo) Get(ctx context.Context, id int64) (*domain.Unit, error) {
	...
	var u domain.Unit
	var created string
	if err := row.Scan(&u.ID, &u.FileID, &u.Key, &u.SourceText, &u.Context, &u.MetadataRaw, &created); err != nil {
		return nil, err
	}
	u.CreatedAt, _ = time.Parse(time.RFC3339, created)
	return &u, nil
