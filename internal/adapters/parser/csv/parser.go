...
func (p *Parser) Parse(data []byte) (ports.ParseResult, error) {
    ...
    header, err := r.Read()
    if err != nil { return ports.ParseResult{}, err }
    idx := map[string]int{}
    for i, h := range header {
        idx[strings.ToLower(strings.TrimSpace(h))] = i
    }
    keyIdx, ok := idx["key"]
    if !ok { return ports.ParseResult{}, errors.New("csv missing 'key' column") }
    // Support source column names
    srcIdx := -1
    for _, name := range []string{"source","value","text","default"} {
        if i, ok := idx[name]; ok { srcIdx = i; break }
    }
    if srcIdx == -1 { return ports.ParseResult{}, errors.New("csv missing source column (source/value/text/default)") }
    ctxIdx := -1
    if i, ok := idx["context"]; ok { ctxIdx = i }
    ...
    for {
        ...
        src := rec[srcIdx]
        var c string
        if ctxIdx >= 0 && ctxIdx < len(rec) { c = rec[ctxIdx] }
